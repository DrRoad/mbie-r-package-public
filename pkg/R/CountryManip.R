...
##==========================================================
CountryGroup <- 
   function(x, shorten=TRUE, type="IVSweights", OneChina_first=FALSE){
      ...
      x <- rename.levels(x,
                         orig = c("Bosnia And Herzegovina", "Isle Of Man", "Trinidad And Tobago", 
                                  "Turks And Caicos Islands", "United States Of America", "USA",
                                  "U.S.", "Wallis And Futuna"),
                         new = c("Bosnia and Herzegovina", "Isle of Man", "Trinidad and Tobago", 
                                 "Turks and Caicos Islands", "United States of America", "United States of America", 
      ...
      ##==================final processing==========================
      OtherCountries <- unique(x[!x %in% Countries])
      x <- (as.factor(ifelse(x %in% Countries, as.character(x), "Other")))
      if(length(OtherCountries) > 0) {
         warning(paste0("Countries '",paste(OtherCountries,collapse = " , "),"' wasn't recognised by the country groups and treated as 'other'!\n If this is not appropriate, please contact maintainer of the package {mbie}!"))
      }
